...
*******************************/
	...
	function __construct() {
		...
		if(!file_exists($this->_ci->config->item('PermissionFile')))
		{
			show_error("APN Failed to connect: APN Permission file not found");
		}
		
		$this->pushServer = $this->_ci->config->item('Sandbox') ? $this->_ci->config->item('PushGatewaySandbox') : $this->_ci->config->item('PushGateway');
	//	$this->feedbackServer = $this->_ci->config->item('Sandbox') ? $this->_ci->config->item('FeedbackGatewaySandbox') : $this->_ci->config->item('FeedbackGateway');
		
		$this->keyCertFilePath = $this->_ci->config->item('PermissionFile');
		$this->passphrase = $this->_ci->config->item('PassPhrase');
		$this->timeout = $this->_ci->config->item('Timeout') ? $this->_ci->config->item('Timeout') : 60;
		$this->expiry = $this->_ci->config->item('Expiry') ? $this->_ci->config->item('Expiry') : 86400;
	}
